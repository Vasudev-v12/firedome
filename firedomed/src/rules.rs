...
pub fn check_ip(ip: &str, rules: &[Rule]) -> bool {
    for rule in rules {
        match (&rule.action, &rule.target) {
            (Action::Block, Target::Ip(rule_ip)) if rule_ip == ip => {
                return false;
            }
            (Action::Allow, Target::Ip(rule_ip)) if rule_ip == ip => {
                return true;
            }
            _ => {}
        }
...
pub fn load_rules(path: &str) -> Vec<Rule> {
    ...
    let content = fs::read_to_string(path).expect("Unable to read rules.conf");
